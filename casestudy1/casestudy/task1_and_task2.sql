...
SELECT *FROM nhan_vien WHERE ho_ten LIKE 'H%'OR  ho_ten LIKE   'B%'  OR  ho_ten LIKE 'K%' AND length(ho_ten) <=15;
-- SELECT round(DATEDIFF(CURDATE(),ngay_sinh)/365,0 ) as yearsFROM khach_hang limit 5;
...
SELECT ho_ten FROM nhan_vien WHERE ho_ten regexp'^[HBK]' AND length(ho_ten)<=15;
